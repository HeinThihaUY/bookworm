<?php
  ...
  $start = ($page-1) * $limit; 
  $total = mysqli_query($conn, "SELECT * FROM books WHERE category_id=$id"); 
  $total_records = mysqli_num_rows($total);
  $book= mysqli_query($conn, "SELECT * FROM books WHERE category_id=$id ORDER BY created_date LIMIT $start, $limit");  
  ...
  <body id="page-top">
    ...
    <header class="text-white bg-info">
      <div class="container text-left">
        ...
        <p class="lead"><?php echo $category['summary'] ?></p>
      </div>
    </header>

    <div class="container-fluid mb-3">
      <div class="container-fluid bd-shadow pt-3 pb-1 mt-3">
      <h3><?php echo $category['name'] ?></h3><hr>
      <div class="row">
        <?php 
        ...
        <div class="col-12 col-sm-6 col-md-4 col-lg-3">
            <div class="book-more rounded mb-4">
              ...
              <div class="card-footer bg-grey">
                ...
                <div class="row">
                  <p class="card-text col-6 col-md-6"><a class="btn btn-info" href="add-to-cart.php?id=<?php echo $book_row['id'] ?>&qty=1&location=4&code=<?php echo $category['id'] ?>&page=<?php echo $page ?>">Cart <i class="fa fa-fw fa-cart-plus" aria-hidden="true"></i></a></p>
      ...
      <?php  
        ...
        if($prev < 1){
          $pagLink .= "<li class='page-item disabled'><a class='page-link' href='category-detail.php?page='".$prev."'>Prev</a></li>";
        }else{
          $pagLink .= "<li class='page-item'><a class='page-link' href='category-detail.php?id=".$category['id']."&page=".$prev."'>Prev</a></li>";
        }
        for ($i=1; $i<=$total_pages; $i++) {  
            if($i == $page){
              $pagLink .= "<li class='page-item active'><a class='page-link' href='category-detail.php?id=".$category['id']."&page=".$i."'>".$i."</a></li>"; 
            }else{
              $pagLink .= "<li class='page-item'><a class='page-link' href='category-detail.php?id=".$category['id']."&page=".$i."'>".$i."</a></li>"; 
            }
                      
        };  
        if($next > $total_pages){
            $pagLink .= "<li class='page-item disabled'><a class='page-link' href='category-detail.php?page=".$next."'>Next</a></li>";
        }else{
            $pagLink .= "<li class='page-item'><a class='page-link' href='category-detail.php?id=".$category['id']."&page=".$next."'>Next</a></li>";
        }
        echo $pagLink . "</ul></nav>";  
      ?> 
      </div>
